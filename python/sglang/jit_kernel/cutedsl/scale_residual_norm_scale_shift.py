...
class ScaleResidualNormScaleShift:
    ...
    def kernel(
        ...
        @cute.jit
        def copy_if(src, dst):
            if cutlass.const_expr(isinstance(src, cute.Tensor) and isinstance(src, cute.Tensor)):
...
def fused_scale_residual_norm_scale_shift(
    ...
    if norm_type == "layer" or norm_type == "rms":
        ...
        cute_tensors = [
            ...
            from_dlpack(t, assumed_align=16) if isinstance(t, torch.Tensor) else t
