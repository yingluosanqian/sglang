...
from sglang.jit_kernel.cutedsl.common.reduce import warp_reduce_sum, cta_reduce_sum
...
def apply_layernorm(
    ...
    if cutlass.const_expr(tWrW is not None):
        for i in range(cute.size(tXrX)):
            tNrN[i] = ((tXrX[i] - mean) * factor * tWrW[i] + tBrB[i]).to(tNrN.element_type)
    else:
        for i in range(cute.size(tXrX)):
...
def tensor_slice(
    ...
    bidx: cutlass.Int32,
    bidy: cutlass.Int32,
    len_f: cutlass.Int32 | cutlass.Constexpr,
    ...
    if cutlass.const_expr(len(mV.shape) == 1):
        ...
        gV = mV[bidx, bidy, None]
    elif cutlass.const_expr(len(mV.shape) == 4):
        gV = mV[bidx, bidy // len_f, 0, None]
    else:
