...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def(
      "scale_residual_layernorm_scale_shift(Tensor residual, Tensor x, Tensor? gate, Tensor? norm_weight, "
      "Tensor? norm_bias, Tensor shift, Tensor scale, float eps, bool is_rms_norm, bool force_fp32_norm) "
      "-> (Tensor, Tensor)");
  m.impl("scale_residual_layernorm_scale_shift", torch::kCUDA, &scale_residual_layernorm_scale_shift);
